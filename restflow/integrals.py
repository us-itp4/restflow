...
def symmetrize(v, in_legs, out_legs, k, model,*args):
  '''
  ...
  for i in range(len(perm)):
    ...
    I_array.append((nom, den*int(len(perm))))
  return I_array

def solve(I_array, k, legs_label, ctx, d, n):
  '''
  Calculates the angular integrals of all the labelled graphs and adds them up.
  '''
  if len(legs_label)==1:
    solved_I = [symvec.integrate2(element, k, legs_label[0], ctx, d, n) for element in I_array]
    ...
    solved_I = [symvec.integrate3(element, k, legs_label[0], legs_label[1], ctx, d, n) for element in I_array]
    solved_I = [symvec.integrate_magnitude(element, k, d) for element in solved_I]
